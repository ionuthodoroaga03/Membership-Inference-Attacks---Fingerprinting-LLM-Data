...
for csv_path in csv_files:
    ...
    if num_duplicates > 0:
        # Show a few examples of duplicated texts
        dup_mask = df.duplicated(subset=["text"], keep=False)
        dup_texts = df.loc[dup_mask, "text"].value_counts().head(5)
        print(f"\nTop {len(dup_texts)} most repeated texts:")
    ...
    # Use label_str if available, otherwise fall back to labels
